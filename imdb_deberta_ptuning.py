...
train_ds = datasets.Dataset.from_dict(
    {"label": train_df["sentiment"].tolist(), "text": train_df["review"].tolist()}
)
val_ds = datasets.Dataset.from_dict(
    {"label": val_df["sentiment"].tolist(), "text": val_df["review"].tolist()}
)
test_ds = datasets.Dataset.from_dict({"text": test["review"].tolist()})

# ==== 模型与分词器（本地加载） ====
model_id = r"./models/deberta-v3-base"  # 确认这里是你存模型的路径
...
out_path = "./result/deberta_ptuning.csv"
pd.DataFrame({"id": test["id"], "sentiment": test_pred}).to_csv(
    out_path, index=False, quoting=3
)
